...
export class OracleService {
    ...
    try {
      ...
      if (process.env.ORACLE_OLD_CRYPTO === 'true') {
        ...
        if (libDir) {
          ...
          // Tentar inicializar sem diretório específico
          try {
            console.error('🔍 Tentando inicializar Oracle Thick sem diretório específico...');
            oracledb.initOracleClient();
            console.error('✅ Oracle Thick inicializado corretamente');
          } catch (clientError: any) {
            console.error('⚠️  AVISO: Não foi possível inicializar o Oracle Thick:', clientError.message);
        ...
        // Configurar o diretório do cliente Oracle se estiver especificado (modo Thin)
      ...
      throw new Error(`Erro ao inicializar Oracle DB: ${error.message}`);
    }
  }
  ...
  private async createPool(): Promise<oracledb.Pool> {
    ...
    // Configurações para versões antigas do Oracle
    if (process.env.ORACLE_OLD_CRYPTO === 'true') {
